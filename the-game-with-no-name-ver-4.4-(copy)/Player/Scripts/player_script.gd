...
func check_for_horizontal_movement() -> void:
	...
	if direction == 0:
		...
		if on_floor and not animatedSprite.animation == str("idle_", currentPlayer):
			animatedSprite.play(str("idle_", currentPlayer))
		return
...
func set_animation() -> void:
	...
	if (islaunching or jumping) and not animatedSprite.animation == str("jump_", currentPlayer):
		animatedSprite.play(str("jump_", currentPlayer))
	
	elif falling and not animatedSprite.animation == str("fall_", currentPlayer):
		animatedSprite.play(str("fall_", currentPlayer))
	
	if velocity.x == 0:
		return
	
	if on_floor and not animatedSprite.animation == str("walk_", currentPlayer): 
		animatedSprite.play(str("walk_", currentPlayer))
